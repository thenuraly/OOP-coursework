...
def checkWinner(player):
    ...
    if won:
        ...
        log_game_result(player.upper() + ' wins')  # Log the game result
...
def CompMove():
    ...
    if not move:
        ...
        background = p.transform.scale(background, (WIDTH, HEIGHT))
        log_game_result("It's a tie!")  # Log the game result
...
def log_game_result(result):
    with open('game_results copy.txt', 'a') as file:
        file.write(result + '\n')
...
# After the game loop ends, quit pygame
p.quit()
